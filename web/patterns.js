...
// ("Common Caddyfile Patterns"). Clicking one inserts it at the cursor.
//
// `code` is a CodeMirror 6 snippet template: `#{text}` marks a placeholder whose
// initial content is `text`. Insertion selects the first one and Tab steps
// through the rest, so the stand-in values no longer have to be hunted down by
// eye. Fields sharing a name are edited together — typing the domain once in the
// www-redirect pattern fills every place it appears.
//
// Caddy's own `{uri}`, `{host}`, `{path}` placeholders are untouched: only `#{}`
